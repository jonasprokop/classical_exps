...
def get_size_tuning_curves(
    ...
    ''' This function is a size tuning function that makes the size of circular and annular grating images vary 
        ...
        NB2: The annular stimulus is computed as a true ring with a fixed outer boundary equal to the largest circular aperture that fits inside the image. 
    ...
    with torch.no_grad():
        ...
        gray_stim = torch.ones((1, *img_res)).to(device) * gray_val
        gray_resp = single_model(gray_stim)

        ## Shared geometry for all stimuli
        bounds = BoundingBox(points = ((-size[1]/2, -size[0]/2), (size[1]/2, size[0]/2)))
        x_deg = get_offset_in_degr(x_pix, img_res[1], size[1])
        y_deg = -get_offset_in_degr(y_pix, img_res[0], size[0])

        ## Create the grating background in normalized space [-1, 1]
        ...
        for i, radius in enumerate(radii):
            ...
            radius_eff = float(radius)

            ## Make the circular stimulus
            grating_center = torch.Tensor(imagen.SineGrating(
                mask_shape=imagen.Disk(smoothing=0.0, size=radius_eff * 2.0),
                orientation=preferred_ori,
                frequency=preferred_sf,
                phase=preferred_phase,
                bounds=bounds,
            ...
            radii_for_panel.append(radius_eff)

            grating_center = rescale(grating_center, -1, 1, pixel_min, pixel_max).reshape(1, *img_res).to(device)

            if neg_val == False:
                circular_tuning_curve[i] = torch.maximum(
                    single_model(grating_center) - gray_resp,
            ...
            if compute_annular:
                ## Inner exclusion disk only, no outer circular cap
                inner_disk = torch.Tensor(imagen.Disk(
                    smoothing=0.0,
                    size=radius_eff * 2.0,
                    bounds=bounds,
                    xdensity=img_res[1] / size[1],
                ...
                ring_mask = 1.0 - inner_disk
                grating_ring = grating_background * ring_mask
                ...
                if neg_val == False:
                    annular_tuning_curve[i] = torch.maximum(
                        single_model(grating_ring) - gray_resp,
                        torch.tensor(0, device=device)
                    )
                else:
...
def size_tuning_experiment_all_phases(
    ...
    with h5py.File(h5_file,'a') as file :
        ...
        for neuron_id in tqdm(neuron_ids):
            ...
            if neuron not in subgroup_path_tuning_curves :
                ...
                for phase in phases:
                    circular_tc, annular_tc = get_size_tuning_curves(
                    ...
                    circular_stack.append(torch.as_tensor(circular_tc))
                    annular_stack.append(torch.as_tensor(annular_tc))

                circular_tc_mean = torch.stack(circular_stack, dim=0).mean(dim=0)
                annular_tc_mean  = torch.stack(annular_stack,  dim=0).mean(dim=0)

                # Curves to save
                circular_tc_pref = circular_tc_mean
                annular_tc_pref  = annular_tc_mean
