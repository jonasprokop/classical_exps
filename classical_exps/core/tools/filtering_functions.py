...
def filter_no_supp_neurons(
    ...
    '''
    filtered_neuron_ids = np.array(neuron_ids)
...
def filter_missing_gsf_amrf(
    h5_file,
    neuron_ids,
    print_results=False,
):
    '''
    Return only neurons with valid GSF and AMRF values.
    ...
    NB:
        - AMRF == GSF is allowed
        - this filter is meant as a stimulus-geometry validity check,
          not as an additional suppression-strength criterion

    Prerequisite :
        - size tuning results already computed for the required neurons

    Expected dataset layout in '/size_tuning/results' :
        [GSF, surround_extent, AMRF, SI, Ropt, Rsupp]
    '''

    group_path = '/size_tuning/results'

    ## Check if size tuning results exist
    check_group_exists_error(h5_file=h5_file, group_path=group_path)

    ## Check if the neurons are present in the data
    check_neurons_presence_error(
        h5_file=h5_file,
        list_group_path=[group_path],
    ...
    with h5py.File(h5_file, 'r') as file:
        ...
        for neuron_id in neuron_ids:
            ...
            neuron = f"neuron_{neuron_id}"
            results = np.asarray(file[group_path][neuron][:], dtype=float).ravel()
            ...
            valid_gsf = np.isfinite(GSF)
            valid_amrf = np.isfinite(AMRF)

            if valid_gsf and valid_amrf:
                filtered_neuron_ids.append(neuron_id)
    ...
    if print_results:
        ...
        print("--------------------------------------")
        print("Filtering neurons with missing GSF / AMRF :")
        print(f"    > There were initially {n} neurons")
        print(f"    > {n-new_n} neurons were removed ({round(100*((n-new_n)/n),1)}%)")
    ...
    return np.asarray(filtered_neuron_ids, dtype=int)
